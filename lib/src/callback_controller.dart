...
///
/// **NEW in v1.2.0:** Adds execution strategies for handling concurrent async operations.
///
...
///
/// **⚠️ IMPORTANT:**
/// - Enqueue mode can grow queue if calls added faster than processed
/// - Replace mode only works if async operation checks for cancellation
...
class ConcurrentAsyncThrottler {
  ...
  int _latestCallId = 0; // Track calls for replace/keepLatest modes
  ...
  Future<void> _processQueue() async {
    ...
    _throttler._debugLog('Started queue processing');

    while (_queue.isNotEmpty) {
      final action = _queue.removeFirst();
      _throttler
          ._debugLog('Processing queued item (${_queue.length} remaining)');

      try {
        // Use AsyncThrottler to execute each queued action
        await _throttler.call(action);
      } catch (e) {
        _throttler._debugLog('Queue processing error: $e');
        // Continue processing queue even if one item fails
      }
    }

    _isProcessingQueue = false;
    _throttler._debugLog('Queue processing complete');
  }
  ...
  Future<void> _replaceCall(Future<void> Function() callback) async {
    // Increment ID to invalidate previous calls
    final currentCallId = ++_latestCallId;
    _throttler._debugLog('Replace mode: new call ID $currentCallId');
    ...
    await _throttler.call(() async {
      ...
      // Check again after execution in case replaced during async operation
      if (currentCallId != _latestCallId) {
        _throttler._debugLog(
  ...
  Future<void> _keepLatestCall(Future<void> Function() callback) async {
    ...
    if (_throttler.isLocked) {
      ...
      _throttler._debugLog('Kept latest call (will execute after current)');
      return;
    }
    ...
    if (_latestCall != null) {
      ...
      _latestCall = null;
      _throttler._debugLog('Executing pending latest call');
  ...
  /// Wraps callback for widget builders.
  VoidCallback? wrap(Future<void> Function()? callback) {
    if (callback == null) return null;
    return () => call(callback);
  }
  ...
  /// Check if there are pending operations in queue (enqueue mode only).
  ...
  void reset() {
    ...
    _isProcessingQueue = false;
    _latestCallId++; // Invalidate all pending calls
    _throttler._debugLog('ConcurrentAsyncThrottler reset (mode: ${mode.name})');
  }
